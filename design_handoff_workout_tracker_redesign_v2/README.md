...
--hair:      #2e2e2e   /* primary borders / dividers */
--hair-2:    #1f1f1f   /* in-card dividers (softer) */
--text:      #fafafa   /* primary text */
--dim:       #9a9a9a   /* secondary text (8:1 on bg — WCAG AAA) */
--dim-2:     #6a6a6a   /* tertiary text, placeholders (4:1 on bg — WCAG AA Large) */
...
> **Important — readability on real devices.** Earlier values were `--dim: #7a7a7a` and `--dim-2: #4a4a4a`. On a real iPhone in mixed lighting / with auto-brightness reduced, the smaller eyebrow labels (10–11px UPPERCASE) and bottom-tab labels became unreadable — `#4a4a4a` on `#0a0a0a` is only **2:1**, failing WCAG even for large text. The values above are calibrated: `#9a9a9a` gives 8:1 (AAA), `#6a6a6a` gives 4:1 (AA Large). **Do not lower these.** If you need a quieter colour, lower _density_ (smaller / fewer eyebrows) rather than darker text.
...
## Mobile readability — mandatory global CSS

After the Tailwind config, add this block. These three rules fix problems that showed up on a real iPhone in Safari and are NOT optional.

```html
<style>
  /* 1. iOS Safari applies its own opacity to <input> placeholders on top of
        the color you set, so they become much dimmer than you specified.
        Forcing opacity:1 and -webkit-text-fill-color makes the placeholder
        actually render at the dim-2 value. */
  input::placeholder,
  textarea::placeholder {
    color: #6a6a6a;                  /* var(--dim-2) */
    opacity: 1;
    -webkit-text-fill-color: #6a6a6a;
  }

  /* 2. Apple emoji desaturate visibly on pure black. The wellbeing scale
        (💀 😣 😐 🙂 🔥) needs both a larger size AND a contrast bump to
        stay punchy on phones with auto-brightness lowered. */
  .wellbeing-emoji {
    font-size: 28px;
    line-height: 1;
  ...
  /* 3. Inactive bottom-tab labels were unreadable at #4a4a4a (2:1 contrast).
        Use #8a8a8a (~6:1) for inactive, pure white for active. */
  .tab-inactive { color: #8a8a8a; }
  .tab-active   { color: #ffffff; }
</style>
```

Apply `.wellbeing-emoji` to every wellbeing emoji on every screen (workout form, workout list rows, workout detail header, client recent workouts). Apply `.tab-inactive` / `.tab-active` to the bottom-tab labels and the matching icon strokes.

### Why these contrast values

| Colour            | On `#0a0a0a` | WCAG          | Use                                       |
|-------------------|--------------|---------------|-------------------------------------------|
| `#4a4a4a` (old)   | 2.05:1       | ❌ fails AA   | — do not use                              |
| `#6a6a6a` (new dim-2) | 4.0:1    | ✅ AA Large   | placeholders, tertiary chips              |
| `#7a7a7a` (old)   | 5.27:1       | ⚠️ marginal AA | — was used for eyebrows, replaced         |
| `#8a8a8a`         | 6.4:1        | ✅ AA          | inactive tab labels and icons             |
| `#9a9a9a` (new dim) | 8.0:1      | ✅ AAA        | all eyebrow labels, secondary copy, meta  |
